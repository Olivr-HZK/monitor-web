...
**前端如何拿数据（登录后）**：浏览器请求 `GET /api/data/{相对路径}`。根目录 canonical `.db`（如 `wechatdouyin.db`、`sensortower_top100.db`）由后端映射到各上游项目源库，并按请求生成 SQLite backup 快照返回；其它 JSON/CSV/MD/图片仍从仓库 **`public/`** 下读取。安全边界：路径不得含 `..`，静态路径必须解析在 `public` 内，部分敏感 basename（如 `auth-config.json`）不返回。详见 **`agent-frontend-data-pipeline.md`**。

**服务端工具 `query_sqlite`**：对后端可用的 canonical `*.db` 做只读 `SELECT` / `WITH` / `PRAGMA table_info(表名)`（实现见 `backend/ai_tools.py`）；优先使用 `backend/config.py` 的 `DATA_SOURCE_DB_PATHS` 直连源库，`public/*.db` 仅作为兼容回退；`db` 参数只能是**文件名**，不含子路径。
...
| `us_free_appid_weekly.db` | 我方产品 US 免费榜日总结与按产品追溯（`weekly_summaries` / `app_ranks`），源库为 `sensortower-/data/us_free_appid_weekly.db` |

**每周出海周报（JSON，非 SQLite）**：`public/休闲游戏检测/出海周报/` 下 `weekly_report_*.json` 与 `index.json`（Puzzle Game 海外市场，同步 game daily report2）。助手应使用 **`read_public_report`**（`path=latest` 或指定期数），**不要**用 `query_sqlite`。
