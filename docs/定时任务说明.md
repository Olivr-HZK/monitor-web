...
| **本仓库：同步四库 + 部署 + 推送** | 拷库 → `npm run deploy` → **三条**周报推送（与 `run_all_latest_reports.sh` 一致：ST / 竞品社媒 / 微信抖音） | **`monitor-web/scripts/sync_dbs_and_deploy.sh`**（建议 **每周一 08:00**，见下；四库含我方产品 `us_free_appid_weekly.db`） |
...
|----------|------|----------|------|-----------|
| **08:00**（需自行加入 crontab） | 每周一 | 备份 → 同步四库 → 部署 → **三条**推送（ST / 竞品社媒 / 微信抖音） | `monitor-web/scripts/sync_dbs_and_deploy.sh` | `monitor-web/logs/sync_dbs_cron.log`（若配置）、`monitor-web/logs/sync_dbs_and_deploy_YYYY-MM-DD.log` |
...
| SensorTower「我方产品」日更（**8:30**，见本机 crontab） | `sensortower/data/us free app id.db`（优先）或 `sensortower/data/us_free_appid_weekly.db`；同步到 `monitor-web/public/us_free_appid_weekly.db`。**周一 8:00 同步早于 8:30 日更**则该趟不含当日我方产品日更写入 |
| 竞品周报（周一 7:40） | 写回 `competitor_data.db` 等，并可能推送；脚本末尾可 `cp` 至 `monitor-web/public/competitor_data.db` |
| 同步+部署（周一 **08:00**，若配置） | 从四处上游拷库到 `monitor-web/public/`（含我方产品库）→ `npm run deploy` → `send_sensortower_weekly_push` / `send_competitor_social_weekly_push` / `send_wechat_douyin_weekly_push` |

---

## 二、任务依赖与联系

1. **monitor-web 的 `sync_dbs_and_deploy.sh`** 依赖四份上游库在当日 cutoff（**默认今日 7:00 之后**有修改，可用 `SYNC_CUTOFF_HOUR` 覆盖）已更新；脚本内会轮询最多约 90 分钟。周一 **7:30～7:40** 的上游一般可满足 **8:00** 同步。「我方产品」日更在 **8:30** 时，**8:00** 同步**不会**包含该次写入；若需包含，请把同步改到 **8:35 之后**（或调 cutoff / 日更时刻）。
...
```cron
# 每周一 08:00：备份 → 拷四库 → 部署 GitHub Pages → 三条周报推送（ST / 竞品社媒 / 微信抖音）
