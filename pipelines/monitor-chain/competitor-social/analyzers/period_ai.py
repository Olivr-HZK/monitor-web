...
def build_period_analysis_prompt(
    ...
    """
    构建时间段分析提示词（按公司维度：该公司该时间段内所有社媒帖子一起分析）。
    重点：新玩法、线下活动；无则写无；记录对应帖子 URL；日常维护一笔带过；保留建议动作；不包含 UA 灵感。
    """
    if not all_posts:
        return f"""你是一个资深的游戏发行与投放总监，专门帮团队做「竞品社媒监控」。

现在给你的是竞品公司【{company}】在社交媒体上过去 {period_days} 天内的监控结果：

---
【竞品公司】{company}
【时间段】过去 {period_days} 天
【状态】该公司在过去 {period_days} 天内无任何社媒更新
---
...
{{
  ...
  "summary": "该公司在过去 {period_days} 天内无社媒更新，建议手动查看各平台了解情况。",
  "new_gameplay": "无",
  "new_gameplay_post_urls": [],
  "offline_events": "无",
  "offline_event_post_urls": [],
  "routine_note": "",
  "direct_action_suggestions": "建议手动查看各平台账号了解状态和可能的原因。"
}}"""

    # 构建帖子列表（每条带链接，便于模型标注哪些是新玩法/线下活动）
    posts_block = []
    for i, post in enumerate(all_posts[:80], 1):
        ...
        if post_url:
            label += f" 链接: {post_url}"
        posts_block.append(f"{label}\n平台: {platform}" + (f" 游戏: {game}" if game else "") + f"\n时间: {pub}\n内容: {text}")
...
现在给你的是竞品公司【{company}】在该时间段内、**所有平台**上的社媒更新帖子（已合并在一起），请整体分析并做一个 summary。
...
分析要求：
1. **摘要**：用一段话总结该公司在这段时间内社媒上的整体动态（summary）。
2. **新玩法**：是否出现新玩法、新功能、新机制、新活动等产出。
   - 如果没有提到任何新玩法相关内容，必须只写「无」，不要写其他解释。
   - 如果有，请简要说明，并在 new_gameplay_post_urls 中列出**对应帖子的完整 URL**（上面每条帖子都有「链接:」）。
3. **线下活动**：是否出现线下活动、展会、发布会、合作活动等。
   - 如果没有提到任何线下活动，必须只写「无」，不要写其他解释。
   - 如果有，请简要说明，并在 offline_event_post_urls 中列出**对应帖子的完整 URL**。
4. **日常维护**：如果该周更新大多为日常维护内容（普通宣传、节日问候、常规更新等），则只简单一笔带过，写在 routine_note 里；否则可留空。
5. **建议动作**：给团队的可执行建议（保留此模块），列表 3–6 条。
6. **周报评分**：根据「是否有新玩法、是否有线下活动」打 1–10 分。有新玩法或线下活动打高分（7–10），只有其一 5–7；都没有或多为日常维护打 1–3 分。**只要有帖子更新就不能打 0 分**，0 分仅用于零更新。
7. **周报标题**：根据本周真实动向写**一句具体标题**（例如「新玩法上线 + 线下展会预告」「日常运营为主，无重大动作」「多平台常规内容更新」）。**禁止使用「本周社媒动态」等笼统表述**，必须写出本周的具体情况。

请严格按以下 JSON 结构输出（不要出现多余字段或自然语言）：

{{
  "company": "{company}",
  "weekly_score": 1-10 的整数（有更新则至少 1 分；有新玩法/线下活动则 5-10）,
  "weekly_title": "一句具体标题，概括本周真实动向（禁止写「本周社媒动态」）",
  "summary": "一段话总结该公司该时间段内社媒整体动态。",
  "new_gameplay": "无 或 简要描述新玩法/新功能/新活动（若无则必须写「无」）",
  "new_gameplay_post_urls": ["对应帖子的完整URL列表，若无则为空数组"],
  "offline_events": "无 或 简要描述线下活动（若无则必须写「无」）",
  "offline_event_post_urls": ["对应帖子的完整URL列表，若无则为空数组"],
...
def analyze_company_period_data(
    ...
    """
    按公司分析：将该时间段内该公司所有平台的帖子合并后做一次分析。

    Args:
        company: 公司名称
        platforms_data_list: 该公司下各平台、各日期的数据列表（每项含 posts, platform_type, game 等）
        period_days: 时间段天数

    Returns:
        AI 分析结果（含 summary, new_gameplay, new_gameplay_post_urls, offline_events, offline_event_post_urls, routine_note, direct_action_suggestions），失败返回 None
    """
    ...
    return {
        ...
        "new_gameplay": data.get("new_gameplay") or "无",
        "new_gameplay_post_urls": _norm_urls(data.get("new_gameplay_post_urls")),
        "offline_events": data.get("offline_events") or "无",
        "offline_event_post_urls": _norm_urls(data.get("offline_event_post_urls")),
...
def analyze_extracted_data(extracted_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    分析提取的数据。按公司维度：该公司该时间段内所有社媒帖子合并后做一次分析。
        ...
        分析结果，格式：
        {
            ...
            "companies": {
                "company_name": {
                    ...
                    "company_analysis": {
                        "summary", "new_gameplay", "new_gameplay_post_urls",
                        "offline_events", "offline_event_post_urls", "routine_note",
