...
def build_company_period_feishu_card(
    ...
    """
    构建公司时间段日报的飞书卡片（按公司维度：摘要、新玩法/线下活动及可点击链接、日常说明、建议动作）。

    Args:
        company: 公司名称
        period: 时间段信息 {"start_date": "...", "end_date": "...", "days": 7}
        company_analysis: 该公司该时间段的 AI 分析结果（summary, new_gameplay, new_gameplay_post_urls, offline_events, offline_event_post_urls, routine_note, direct_action_suggestions）
    ...
    if not company_analysis:
        ...
        new_gameplay = company_analysis.get("new_gameplay") or "无"
        new_gameplay_urls = company_analysis.get("new_gameplay_post_urls") or []
        offline_events = company_analysis.get("offline_events") or "无"
        offline_urls = company_analysis.get("offline_event_post_urls") or []
        ...
        # 新玩法：无则写无；有则说明 + 可点击链接
        gameplay_block = f"🎮 **新玩法**: {new_gameplay}"
        if new_gameplay_urls:
            gameplay_block += "\n相关帖子链接:\n" + _format_post_urls_as_links(new_gameplay_urls)
        content_lines.append(gameplay_block)

        # 线下活动：无则写无；有则说明 + 可点击链接
        offline_block = f"🎪 **线下活动**: {offline_events}"
        if offline_urls:
            offline_block += "\n相关帖子链接:\n" + _format_post_urls_as_links(offline_urls)
        content_lines.append(offline_block)
