...
def render_html_content(
    ...
    *,
    ...
    html += """</span>
            ...
            <div class="content">
                <div class="report-filter-bar" role="search">
                    <label for="report-filter-input" class="sr-only">按关键词筛选条目</label>
                    <input type="search" id="report-filter-input" class="report-filter-input" placeholder="筛选标题（防抖，仅隐藏不匹配条目）" autocomplete="off" enterkeyhint="search" />
                    <button type="button" class="report-filter-clear">清除</button>
                </div>"""
    ...
    html += """
        ...
        <script>
            ...
            function debounceReportFilter(fn, ms) {
                let timer = null;
                return function () {
                    clearTimeout(timer);
            ...
            let reportFilterReady = false;
            function initReportFilter() {
                if (reportFilterReady) return;
                const container = document.querySelector('.container');
                const input = document.getElementById('report-filter-input');
                const clearBtn = document.querySelector('.report-filter-clear');
                if (!container || !input) return;
                reportFilterReady = true;

                const itemSelectors = '.news-item, .rss-item, .new-item';
                const items = Array.from(container.querySelectorAll(itemSelectors));
                for (let i = 0; i < items.length; i++) {
                    const el = items[i];
                    el.dataset.searchNorm = (el.textContent || '').replace(/\\s+/g, ' ').trim().toLowerCase();
                }

                const wordGroups = Array.from(container.querySelectorAll('.word-group'));
                const standaloneGroups = Array.from(container.querySelectorAll('.standalone-group'));
                const feedGroups = Array.from(container.querySelectorAll('.feed-group'));
                const newSourceGroups = Array.from(container.querySelectorAll('.new-source-group'));
                const rssSections = Array.from(container.querySelectorAll('.rss-section'));
                const newSectionEl = container.querySelector('.new-section');
                const hotlistSection = container.querySelector('.hotlist-section');
                const standaloneSection = container.querySelector('.standalone-section');
                const aiSection = container.querySelector('.ai-section');
                if (aiSection) {
                    aiSection.dataset.searchNorm = (aiSection.textContent || '').replace(/\\s+/g, ' ').trim().toLowerCase();
                }
                ...
                function applyReportFilter() {
                    ...
                    const run = function () {
                        if (!q) {
                            for (let j = 0; j < items.length; j++) items[j].hidden = false;
                            for (let j = 0; j < wordGroups.length; j++) wordGroups[j].hidden = false;
                            for (let j = 0; j < standaloneGroups.length; j++) standaloneGroups[j].hidden = false;
                            for (let j = 0; j < feedGroups.length; j++) feedGroups[j].hidden = false;
                            for (let j = 0; j < newSourceGroups.length; j++) newSourceGroups[j].hidden = false;
                            for (let j = 0; j < rssSections.length; j++) rssSections[j].hidden = false;
                            if (newSectionEl) newSectionEl.hidden = false;
                            if (hotlistSection) hotlistSection.hidden = false;
                            if (standaloneSection) standaloneSection.hidden = false;
                            if (aiSection) aiSection.hidden = false;
                            return;
                        }
                        for (let j = 0; j < items.length; j++) {
                            const el = items[j];
                            el.hidden = el.dataset.searchNorm.indexOf(q) === -1;
                        }
                        for (let j = 0; j < wordGroups.length; j++) {
                            const g = wordGroups[j];
                            const list = g.querySelectorAll('.news-item');
                            let vis = false;
                            for (let k = 0; k < list.length; k++) {
                                if (!list[k].hidden) { vis = true; break; }
                            }
                            g.hidden = !vis;
                        }
                        for (let j = 0; j < standaloneGroups.length; j++) {
                            const g = standaloneGroups[j];
                            const list = g.querySelectorAll('.news-item');
                            let vis = false;
                            for (let k = 0; k < list.length; k++) {
                                if (!list[k].hidden) { vis = true; break; }
                        ...
                        for (let j = 0; j < feedGroups.length; j++) {
                            const g = feedGroups[j];
                            const list = g.querySelectorAll('.rss-item');
                            let vis = false;
                            for (let k = 0; k < list.length; k++) {
                                if (!list[k].hidden) { vis = true; break; }
                            }
                        ...
                        for (let j = 0; j < newSourceGroups.length; j++) {
                            const g = newSourceGroups[j];
                            const list = g.querySelectorAll('.new-item');
                            let vis = false;
                            for (let k = 0; k < list.length; k++) {
                                if (!list[k].hidden) { vis = true; break; }
                            }
                        ...
                        for (let j = 0; j < rssSections.length; j++) {
                            const sec = rssSections[j];
                            const list = sec.querySelectorAll('.feed-group');
                            let vis = false;
                            for (let k = 0; k < list.length; k++) {
                                if (!list[k].hidden) { vis = true; break; }
                            }
                        ...
                        if (newSectionEl) {
                            const list = newSectionEl.querySelectorAll('.new-source-group');
                            let vis = false;
                            for (let k = 0; k < list.length; k++) {
                                if (!list[k].hidden) { vis = true; break; }
                            }
                            newSectionEl.hidden = !vis;
                        }
                        if (hotlistSection) {
                            const list = hotlistSection.querySelectorAll('.word-group');
                            let vis = false;
                            for (let k = 0; k < list.length; k++) {
                                if (!list[k].hidden) { vis = true; break; }
                            }
                            hotlistSection.hidden = !vis;
                        }
                        if (standaloneSection) {
                            const list = standaloneSection.querySelectorAll('.standalone-group');
                            let vis = false;
                            for (let k = 0; k < list.length; k++) {
                                if (!list[k].hidden) { vis = true; break; }
                            }
                            standaloneSection.hidden = !vis;
                        }
                        if (aiSection && aiSection.dataset.searchNorm) {
                            aiSection.hidden = aiSection.dataset.searchNorm.indexOf(q) === -1;
                        }
                    };
                ...
                input.addEventListener('input', debounceReportFilter(applyReportFilter, 120));
                input.addEventListener('search', applyReportFilter);
                if (clearBtn) {
