...
def format_date_folder(
    ...
    if date:
        return date
    override_date = os.getenv("TRENDRADAR_DATE_OVERRIDE") or os.getenv(
        "GAMING_DAILY_DATE_OVERRIDE"
    )
    if override_date:
        try:
            return datetime.strptime(override_date, "%Y-%m-%d").strftime("%Y-%m-%d")
        except ValueError:
            print(f"[警告] 日期覆盖值无效: {override_date}，将使用当前日期")
