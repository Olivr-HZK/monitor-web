...
const GameRankingTable = ({ items, rankingType }: GameRankingTableProps) => {
  const getRankChangeDisplay = (change: string) => {
    ...
    // 其他情况直接显示
    return (
        <span className="text-slate-200 font-medium">{change}</span>
    );
  };
        ...
        <table className="w-full min-w-[800px]">
          <thead>
            <tr className="border-b border-slate-800 bg-slate-900/80">
              <th className="text-left py-4 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider">排名</th>
          ...
          <tbody className="divide-y divide-slate-800">
            {items.map((item, index) => (
              <tr
                key={item.id}
                className={`hover:bg-slate-800/60 transition-colors ${index % 2 === 0 ? 'bg-slate-950/40' : 'bg-slate-900/40'}`}
              >
                <td className="py-4 px-6 whitespace-nowrap">
                  <span className="inline-flex items-center justify-center w-10 h-10 rounded-lg text-sm font-bold bg-slate-800 text-slate-200">
                    {item.rank}
                  </span>
                </td>
          ...
          <tbody className="divide-y divide-slate-800">
            {items.map((item, index) => (
              ...
              >
                ...
                <td className="py-4 px-6 text-right whitespace-nowrap text-sm font-medium text-slate-300">
                  {item.downloads || '—'}
                </td>
                <td className="py-4 px-6 text-right whitespace-nowrap text-sm font-medium text-white">
    ...
    return (
      <div className="overflow-x-auto -mx-6">
        <table className="w-full min-w-[1200px]">
          <thead>
            <tr className="border-b border-slate-800 bg-slate-900/80">
              <th className="text-left py-4 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider">信号</th>
          ...
          <tbody className="divide-y divide-slate-800">
            {items.map((item, index) => (
              ...
              >
                <td className="py-4 px-6 whitespace-nowrap text-lg text-slate-200">
                  {item.signal || '—'}
                </td>
                <td className="py-4 px-6">
                  <div className="font-semibold text-white text-base">{item.name}</div>
                ...
                <td className="py-4 px-6 text-right whitespace-nowrap font-semibold text-white">
                  {item.rank}
                </td>
                <td className="py-4 px-6 text-right whitespace-nowrap text-sm text-slate-300">
                  {item.lastRankRaw || '-'}
                </td>
                <td className="py-4 px-6 text-center whitespace-nowrap">
                  {getRankChangeDisplay(item.change)}
        ...
        <tbody className="divide-y divide-slate-800">
          {items.map((item, index) => (
            ...
            >
              ...
              <td className="py-4 px-6">
                <span className="text-sm text-slate-300">{item.appId || '—'}</span>
              </td>

              <td className="py-4 px-6 whitespace-nowrap">
                <span className="text-sm text-slate-300">{item.platformLabel || '—'}</span>
              </td>
              ...
              <td className="py-4 px-6 whitespace-nowrap">
                <span className="text-sm text-slate-300">{item.listType || '—'}</span>
              </td>
              <td className="py-4 px-6 whitespace-nowrap">
